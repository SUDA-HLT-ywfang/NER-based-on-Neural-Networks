...
class Trident_Transfer_Net(nn.Module):
    def __init__(self, data):
        ...
        # Feature Encoder
        self.common_lstm = Encoder(
            feature_extractor='BiLSTM',
            embedding_dim=data.word_emb_dim,
            hidden_dim=data.common_hidden_dim,
            num_layers=1)
    ...
    def forward(self, batch):
        if self.training:
            ...
            common_feature4source = self.common_lstm(sorted_source_word_present, sorted_source_sentlength)
            common_feature4target = self.common_lstm(sorted_target_word_present, sorted_target_sentlength)
            common_feature4source = self.common_lstm_dropout(common_feature4source)
            common_feature4target = self.common_lstm_dropout(common_feature4target)
            ...
            common_feature = self.common_lstm(sorted_wordemb, sorted_sentlength)
