# BiLSTM-CRF模型的训练
import argparse
from utils.corpus import Corpus_With_Multi_Label, Corpus
from utils.vocab import Vocab, Label_Vocab_for_partialcrf
...
if __name__ == '__main__':
    ...
    # 局部标注语料，训练集多标签，但是开发和测试是确定的、单标签
    train_samples = Corpus_With_Multi_Label(args.train, do_lower=False, number_normal=True)
    ...
    label_vocab = Label_Vocab_for_partialcrf(train_samples.samples+dev_samples.samples+test_samples.samples)

    train_data = instance_multi_label2tensor(train_samples.samples, word_vocab, char_vocab, label_vocab)
