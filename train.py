...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', required=True,
                        help='filepath of train corpus')
    parser.add_argument('--dev', required=True, help='filepath of dev corpus')
    parser.add_argument('--test', required=True,
                        help='filepath of test corpus')
    parser.add_argument('--pretrain_emb', default=None,
                        help='filepath of pretrained embedding')
    parser.add_argument('--save_model_dir', default='save/best.baseline.model',
                        help='path of saved network parameters')
    parser.add_argument('--save_model_info_dir',
                        default='save/bestmodel_info.model', help='path of saved info for decode')
    parser.add_argument('--epoch_num', default=100, type=int,
                        help='epoch num of model training')
    parser.add_argument('--batch_size', default=10, type=int,
                        help='batch size when training')
    parser.add_argument('--hidden_dim', default=200,
                        type=int, help='hidden dimension of lstm')
    parser.add_argument('--optimizer', default='SGD', help='')
    parser.add_argument('--gpu', type=str, default='-1',
                        help='gpu id, set to -1 if use cpu mode')
    parser.add_argument('--lstm_layers', default=1,
                        type=int, help='lstm layers')
    ...
    train_data = instance2tensor(
        train_samples.samples, word_vocab, char_vocab, label_vocab)
    dev_data = instance2tensor(
        dev_samples.samples, word_vocab, char_vocab, label_vocab)
    test_data = instance2tensor(
        test_samples.samples, word_vocab, char_vocab, label_vocab)
